namespace ScreenFlow.Managers
{
    ...
    using GameFoundation.MVP.CommonScreen;
    ...
    public class ScreenManager : MonoBehaviour, IScreenManager, IDisposable
    {
        ...
        [Inject]
        public void Init(ISignalBus signalBusParam, ILogService logServiceParam, IGameAssets gameAssetsParam, IContainerBuilder containerBuilder, IObjectResolver objectResolver)
        {
        ...
        public async UniTask<T> GetScreen<T>() where T : IScreenPresenter
        {
            ...
            async UniTask<IScreenPresenter> InstantiateScreen()
            {
                ...
                var type = typeof(T);
                this.containerBuilder.RegisterComponentOnNewGameObject(type, Lifetime.Scoped).Build();
                
                screenPresenter = this.resolver.Resolve(type) as IScreenPresenter;
